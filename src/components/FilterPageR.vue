...
const handleSelect = async (value) => {
  try {
    tableData.value = [];
    const response = await axios.get('http://106.15.7.192:8000/evalu/rresult?batch=' + value);
    ...
    for (let i = 0; i < data.length; i++) {
      tableData.value.push({
        ...
        tag: data[i].rresult == null ? 'Safe' :
          (data[i].rresult < 0.5 ? 'Safe' :
            (data[i].rresult < 0.7 ? 'Warning' :
              'Danger'))
      });
    }
